...
def untar_files(tar_file, destination_folder):
    """
    DESCRIPTION: Extracts tar file to destination folder
    INPUT: 1. Tar file, 2. Destination folder
    OUTPUT: -
    """
    try:
...
def untar_files_loop(tar_path, extraction_folder):
    """
    DESCRIPTION: Loops over all tar files and puts extracted folders into an extraction folder
    INPUT: 
    OUTPUT: -
    """
    ...
    for filename in os.listdir(tar_path):
        ...
        destination_folder = os.path.join(extraction_folder, destination_folder_name)
...
def data_reduction_fun(df_eventfiles,df_properties,global_path, min_counts = 5):
    """
